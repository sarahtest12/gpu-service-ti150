#!/usr/bin/env python3
"""Authenticated WebSocket service for CosyVoice-300M-SFT."""
...
LOG = logging.getLogger("tts-300m-sft-service")
...
def load_engine(cfg):
    ...
    if set(model.list_avaliable_spks()) != set(cfg["checkpoint_speakers"]):
        raise RuntimeError("300M-SFT checkpoint speaker metadata mismatch")
...
def create_app(engine, cfg, token):
    app = FastAPI(title="CosyVoice-300M-SFT TTS internal service", version="1.0",
...
def main():
    ...
    LOG.info("TTS 300M SFT ready on ws://%s:%d/realtime", cfg["host"], cfg["port"])
