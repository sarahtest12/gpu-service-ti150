...
| 项目 | 结果 |
| --- | --- |
| SFT 服务、协议与 CPU 客户端测试 | 55 项通过 |
| 300M-Instruct / Fun-CosyVoice3 回归 | 55 项 / 54 项通过 |
...
业务长连接中观察到正常长段之间夹有多条约 0.25–0.65 秒的短音频。独立 GPU 探针把 `。`、
`，`、`：`、`！` 分别送入原始 SFT 推理，模型实际生成了 0.789、0.360、1.358、1.149 秒音频；
ASR 分别转写出“嗯”、空、“一打”和“可以”。正常句子的原始 PCM 只转写出请求正文，因此额外
尾音来自纯标点触发的 speech-token 幻觉，不是 instruction 或播放器追加。

服务现在会过滤归一化后不含字母或数字的内部子段。外部分段仍保持已接受、开始、完成的 FIFO
事件顺序，但不会调用模型或产生 PCM；含汉字、拉丁字母或数字的短句不受影响。SFT 与保留的
300M-Instruct 共用这一修复，并有回归测试覆盖。部署修复后通过统一 TLS/WSS 发送 `。：！`，
实际事件为 `input.accepted`、`audio.start`、`audio.done`，PCM 为 0 字节；同一连接随后合成正常
短句并返回 102912 字节 PCM。重启后的 SFT 进程保持 `ready: true`。
