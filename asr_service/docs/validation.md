...
2026-09-11 增加监控后再次通过统一入口回放 5.546 秒样例。ASR 内部直方图记录 6 次
partial/final 解码，vLLM 首 token 总计约 0.557 秒；同一 60 秒快照中的 avg/P95 为
53.791/78.0 ms。内部 `/metrics` 无 ASR 内部 key 返回 401，统一网关不公开该路径。
