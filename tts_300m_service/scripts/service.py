#!/usr/bin/env python3
"""Validate and run the fixed CosyVoice-300M-Instruct CoreX service."""
...
def config(*, require_artifacts=True):
    ...
    if cfg.get("backend") != "cosyvoice300m":
        raise ValueError("this service supports only cosyvoice300m")
    ...
    if (cfg["model_name"] != "cosyvoice-300m-instruct"
        ...
        raise ValueError("unexpected CosyVoice-300M deployment boundary")
...
print('CosyVoice-300M imports and CoreX runtime: PASS')
"""
...
def main():
    ...
    if args.action == "init-key":
        init_key()
        print(f"TTS 300M internal credential ready: {KEY} (value not displayed)")
        return
    cfg = config()
    if args.action == "check":
        check(cfg)
        print("CosyVoice-300M environment, source and checkpoint: PASS")
        return
    check(cfg)
