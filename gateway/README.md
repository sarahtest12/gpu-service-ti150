...
| `tts_300m_sft_service/runtime/api_key` | 默认 TTS 内部 key，网关注入上游请求 |
...
默认 TTS 配置见 `tts_300m_sft_service/config/server.json`。CosyVoice-300M-SFT 使用 CoreX
FP16 + JIT 分段 FIFO，在 GPU 0 运行。`tts_300m_service` 的 CosyVoice-300M-Instruct 和原
`tts_service` 的 Fun-CosyVoice3 双流服务均保留；三个目录都使用 8004，因此只能运行其中一个。
...
`--service tts` 管理 `gateway/config/server.json` 中 `tts.project` 选中的目录。切换模型时需要先
停止当前 TTS，再同时修改 `tts.project` 和 `tts.api_key_file`，启动目标服务、重启 monitor 并
reload 网关；支持的项目是 `tts_300m_sft_service`、`tts_300m_service` 和 `tts_service`。
monitor 会在启动时读取各算法内部 key，
...
TTS 示例：在 `tts_300m_sft_service/cpu_client/` 运行
